...
export const Sidebar = ({ activePage, onPageChange }: SidebarProps) => {
  ...
  return (
    <aside className="hidden lg:flex w-56 flex-col bg-[#021d18] h-screen fixed left-0 top-0 z-40 border-r border-[#00775B]/15">

      {/* ── Brand / Platform Switcher ── */}
      <div ref={platformRef} className="relative px-3 pt-4 pb-3 border-b border-white/5">
        <button
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto custom-scrollbar">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={cn("h-4 w-4 shrink-0", isActive ? "text-white" : "text-white/40")} />
      ...
      <div className="px-3 pb-4 pt-2 border-t border-white/5 space-y-0.5">
        <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-white/50 hover:text-white/80 hover:bg-white/5 transition-all">
          <HelpCircle className="h-4 w-4 shrink-0 text-white/40" />
